...
static int battery_level = 100;         /* battery percentage (0-100) */
                          ...
                          size_t count, loff_t *ppos)
{
    ...
    /* simulate battery drain: small random decrease per read */
    {
        unsigned int br;
        get_random_bytes(&br, sizeof(br));
        if (battery_level > 0 && (br % 20 == 0))  /* ~5% chance per read */
            battery_level--;
    }
...
static long temp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    case TEMP_IOC_SET_BATTERY: {
        int new_battery;
        if (copy_from_user(&new_battery, (int __user *)arg, sizeof(int)))
            return -EFAULT;
        if (new_battery < 0 || new_battery > 100)
            return -EINVAL;
        mutex_lock(&temp_lock);
        battery_level = new_battery;
        mutex_unlock(&temp_lock);
        pr_info("tempsensor: battery set to %d%%\n", new_battery);
        break;
    }

    case TEMP_IOC_GET_BATTERY:
        mutex_lock(&temp_lock);
        if (copy_to_user((int __user *)arg, &battery_level, sizeof(int))) {
            mutex_unlock(&temp_lock);
            return -EFAULT;
        }
        mutex_unlock(&temp_lock);
        pr_info("tempsensor: battery read as %d%%\n", battery_level);
        break;
